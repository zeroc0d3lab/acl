/**
 * Created by ZeroC0D3 Team on 06/09/2015.
 */
...
class CropAvatar {
    ...
    submit() {
        if (!this.$avatarSrc.val() && !this.$avatarInput.val()) {
            Zeroc0d3lab.showError('Please select image!');
            return false;
        }
    ...
    submitDone(data) {
        ...
        if (data && !data.error) {
            if (data.data) {
                ...
                Zeroc0d3lab.showSuccess(data.message);
            } else {
                Zeroc0d3lab.showError(data.message);
            }
        } else {
            Zeroc0d3lab.showError(data.message);
        }
    ...
    static submitFail(errors) {
        Zeroc0d3lab.handleError(errors);
    }
